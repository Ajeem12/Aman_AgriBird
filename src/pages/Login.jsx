...
const Login = ({ onClose }) => {
  ...
  const handleMobileSubmit = async (e) => {
    e.preventDefault();
    if (!mobile) return toast("Please enter your mobile number");
  ...
  return (
    <div className="w-full h-[70vh] flex items-center justify-center px-2 relative">
      <div className="w-full sm:mt-20 sm:max-w-sm md:max-w-md lg:max-w-lg p-6 sm:p-8 md:p-10 bg-white bg-opacity-80 backdrop-blur-md border border-gray-300 shadow-2xl rounded-2xl mx-4 sm:mx-auto relative transition-all">
        {/* Close Button */}
        {onClose && (
          <button
            onClick={onClose}
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-center text-[#1f4037] mb-4 sm:mb-6">
          {step === 1 ? "Welcome Back!" : "Verify OTP"}
        </h1>
        <p className="text-center text-gray-600 mb-4 sm:mb-6">
          {step === 1
            ...
            <div className="flex justify-center gap-3 mb-4">
              {otp.map((digit, index) => (
                <input
                  ...
                  maxLength="1"
                  className="w-12 h-12 text-center text-xl font-bold border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 transition duration-300 shadow-sm bg-white hover:border-teal-400"
                />
              ))}
            </div>
